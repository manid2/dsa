...
High
	 -> program args
	 -> stack
	 -> free meory | stack pointer meets heap pointer
     -> heap
	 -> bss segment (uninitlized global & static variables)
     -> data segment (initilized global & static variables) -> read, read-write areas
low  -> code .text

// cpp encapsulation & polymorphism

class Node { // encapsulation
private:
	int data;
public:
	void set_data(int a);
...
template <typename T>
class Node { // compile time polymorphism (template, function overloading) + runtime polymorphism (virtual functions)
private:
	T data;
public:
	void set_data(T a);
