...
#define _dij_check_eq(_d, d, _p, p, _p2, p2)                                   \
  if (_d != d)                                                                 \
    _dij_check_eq_fail(_d, d, "distances");                                    \
  if (_p != p)                                                                 \
    _dij_check_eq_fail(_p, p, "parent");                                       \
  if (_p2 != p2)                                                               \
    _dij_check_eq_fail(_p2, p2, "path");                                       \
  SUCCESS();
...
#define _dij_show_output(n, ed, s, t, d, p, p2)                                \
  {                                                                            \
    ...
    om += format("distances = {}, parent = {}, path = {}", to_string(d),       \
                 to_string(p), to_string(p2));                                 \
    SHOW_OUTPUT(im, om);                                                       \
  }
...
#define _DIJ_TEST(var)                                                         \
  TEST(_DIJ_NAME(var), _DIJ_DESC(var)) {                                       \
    ...
    vector<vi2_t> _edges;                                                      \
    _edges.push_back(                                                          \
        {{0, 1, 2}, {0, 3, 1}, {1, 2, 4}, {1, 4, 5}, {2, 3, 3}, {2, 4, 1}});   \
    _edges.push_back({{0, 1, 24}, {0, 3, 20}, {2, 0, 3}, {3, 2, 12}});         \
                                                                               \
    ...
    int n = size(_nodes);                                                      \
    fii(i, n) {                                                                \
      _dij_check(_nodes[i], _edges[i], _sources[i], _targets[i],               \
                 _distances[i], _parent[i], _paths[i], _DIJ_NAME(var));        \
    }                                                                          \
  }
