...
struct _test {
  ...
  template <class... Types> void setIo(Types... args) {
    io = {args...};
  }
  vector<io_t> getIo(void) const {
    return io;
  }
...
// inline prevents global state duplication across multiple translation units
inline vector<_test *> _tests;
...
#define _GET_FAIL_MSG()                                                        \
  cerr << format("expected: {}  actual: {}.", this->em, this->am) << endl
